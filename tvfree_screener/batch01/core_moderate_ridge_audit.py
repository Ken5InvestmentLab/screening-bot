...
FROZEN_SPEC_PATH = REPORT_DIR / "core_moderate_ridge_spec_v4.json"
POLICY_LOCK_PATH = REPORT_DIR / "core_moderate_ridge_discovery_policy_lock_v4.json"
DISCOVERY_REPORT = REPORT_DIR / "core_moderate_ridge_v4_discovery.json"
VALIDATION_REPORT = REPORT_DIR / "core_moderate_ridge_v4_validation_2024.json"
...
def _feature_panel(through: pd.Timestamp, calendar: SessionCalendar) -> pd.DataFrame:
    panel_path = _feature_panel_path(through)
...
def _feature_panel_path(through: pd.Timestamp) -> Path:
    return BATCH_DIR / ".cache" / f"core_moderate_v4_features_through_{through.year}.parquet"


def required_label_signals(
    prepared: pd.DataFrame,
    *,
    score_start: pd.Timestamp,
    score_end: pd.Timestamp,
) -> pd.DataFrame:
    """Keep only labels needed for sampled training and the full score-period pool."""
    required = {"date", "symbol", "close", "session_index"}
    if not required.issubset(prepared.columns):
        raise ValueError("prepared features lack the fields required to scope label computation")
    training = (
        prepared["date"].between(DATA_START, score_end)
        & prepared["session_index"].mod(5).eq(0)
    )
    score_period = prepared["date"].between(score_start, score_end)
    signals = prepared.loc[training | score_period, ["date", "symbol", "close"]].copy()
    return signals.sort_values(["symbol", "date"], kind="mergesort").reset_index(drop=True)
...
def build_labels_for_eligible_signals(
    ...
    price_group_iterator = iter(prices.groupby("symbol", sort=True, observed=True))
    current_price_symbol, current_price_group = next(price_group_iterator, (None, None))
    outputs: list[pd.DataFrame] = []

    for symbol, signal_group in signals.groupby("symbol", sort=True, observed=True):
        symbol = str(symbol)
        while current_price_symbol is not None and str(current_price_symbol) < symbol:
            current_price_symbol, current_price_group = next(price_group_iterator, (None, None))
        price_group = (
            current_price_group
            if current_price_symbol is not None and str(current_price_symbol) == symbol
            else None
        )
...
def run_period(period: str) -> dict[str, object]:
    ...
    panel_path = _feature_panel_path(last_session)
    ...
    del panel
    gc.collect()
    label_signals = required_label_signals(
        prepared,
        score_start=score_start,
        score_end=period_end,
    )
    prices, price_receipt = read_verified_parquet_columns(panel_path, PRICE_COLUMNS)
    if (
        price_receipt.get("source_sha256") != frozen["source_sha256"]
        or price_receipt.get("calendar_sha256") != frozen["calendar_sha256"]
        or price_receipt.get("through") != last_session.date().isoformat()
        or price_receipt.get("labels_included") is not False
    ):
        raise ValueError("projected OHLCV columns do not match the frozen decision-only panel")
    labels = build_labels_for_eligible_signals(prices, label_signals, calendar_obj)
    del prices, label_signals
    gc.collect()
    ...
    report = {
        ...
        "feature_panel_rows": feature_panel_rows,
        "eligible_rows_before_period_purge": eligible_rows,
    ...
    pool_receipt = write_parquet_artifact(
        ...
        BATCH_DIR / ".cache" / f"core_moderate_v4_{period}_candidate_pool.parquet",
        metadata={
            "experiment_id": FAMILY_SPEC["experiment_id"],
            "spec_sha256": frozen["spec_sha256"],
            "source_sha256": frozen["source_sha256"],
    ...
    labels_receipt = write_parquet_artifact(
        ...
        BATCH_DIR / ".cache" / f"core_moderate_v4_{period}_labels.parquet",
        metadata={
            "experiment_id": FAMILY_SPEC["experiment_id"],
            "spec_sha256": frozen["spec_sha256"],
            "source_sha256": frozen["source_sha256"],
    ...
    model_directory = BATCH_DIR / ".cache" / "core_moderate_ridge_v4_models" / period
    model_hashes = {}
    ...
    for top_n, policy_artifacts in artifacts.items():
        receipt = write_parquet_artifact(
            ...
            BATCH_DIR / ".cache" / f"core_moderate_v4_{period}_selected_top{top_n}.parquet",
