...
def freeze_metadata_snapshot(input_dir: str | Path, *, start: object = DEFAULT_START.isoformat(), end: object = DEFAULT_END.isoformat()) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    if missing:
        raise ValueError(f"incomplete EDINET metadata date coverage; missing {len(missing)} day(s): {', '.join(missing[:5])}")
    ...
    for source_date in expected_dates:
        ...
        daily_files.append({"date": source_date.isoformat(), "file": path.name, "sha256": _sha256_bytes(raw), "result_rows": len(results)})
        for idx, item in enumerate(results):
            ...
            if not doc_type_code or not submit_datetime:
                if _is_edinet_null_tombstone(item):
                    excluded_tombstones.append("|".join([source_date.isoformat(), doc_id, _optional_text(item.get("withdrawalStatus")), _optional_text(item.get("parentDocID"))]))
                    continue
                raise ValueError(f"{source_date}: result row {idx} missing docID/docTypeCode/submitDateTime")
            try:
            ...
            rows.append({"doc_id": doc_id, "doc_type_code": doc_type_code, "submit_datetime": submit_ts, "source_date": source_date.isoformat()})

    frame = pd.DataFrame(rows, columns=["doc_id", "doc_type_code", "submit_datetime", "source_date"])
    duplicate_doc_ids = duplicate_observation_rows = eligible_duplicate_doc_ids = 0
    duplicate_material = []
    if not frame.empty and frame["doc_id"].duplicated().any():
        ...
        for doc_id, group in duplicated.groupby("doc_id", sort=True):
            # Compare normalized instants, not string rendering. pandas may render
            # the same Asia/Tokyo instant as +09:00 or +09:18 for historical
            # timezone data; string comparison would create a false conflict.
            submit_instants = pd.to_datetime(group["submit_datetime"], utc=True)
            if int(submit_instants.nunique()) != 1:
                preview = sorted(group["submit_datetime"].map(lambda x: x.isoformat()).unique().tolist())
                raise ValueError(f"duplicate doc_id has conflicting submit_datetime: {doc_id} -> {preview[:3]}")
            ...
            if eligible_types:
                ...
                if len(doc_types) != 1:
                    raise ValueError(f"duplicate eligible doc_id has conflicting doc_type_code: {doc_id} -> {doc_types}")
            for row in group.sort_values(["source_date", "doc_type_code", "submit_datetime"], kind="stable").itertuples(index=False):
                duplicate_material.append("|".join([str(row.doc_id), str(row.source_date), str(row.doc_type_code), pd.Timestamp(row.submit_datetime).isoformat()]))

    if not frame.empty:
        frame["submit_datetime"] = frame["submit_datetime"].map(lambda x: pd.Timestamp(x).isoformat())
    frame = frame.sort_values(["submit_datetime", "doc_id", "doc_type_code", "source_date"], kind="stable").reset_index(drop=True)
    aggregate_material = "\n".join(f"{entry['date']} {entry['sha256']}" for entry in daily_files).encode("utf-8")
    manifest = {
        "status": "EDINET_METADATA_SNAPSHOT_COMPLETE_OUTCOME_BLIND",
        "coverage": {"start": start_date.isoformat(), "end": end_date.isoformat(), "expected_calendar_days": len(expected_dates), "observed_calendar_days": len(daily_files), "full_period_pass": True},
        ...
        "excluded_document_list_rows": {"edinet_null_tombstone_count": len(excluded_tombstones), "edinet_null_tombstone_sha256": _sha256_bytes("\n".join(excluded_tombstones).encode("utf-8")), "policy": "skip only inert EDINET document-list rows with docTypeCode=null, disclosureStatus=0, and all content/legal flags=0; submitDateTime may be null or retained; all other malformed rows fail closed"},
        "duplicate_document_observations": {"duplicate_doc_id_count": duplicate_doc_ids, "duplicate_observation_rows": duplicate_observation_rows, "eligible_duplicate_doc_id_count": eligible_duplicate_doc_ids, "duplicate_observation_sha256": _sha256_bytes("\n".join(duplicate_material).encode("utf-8")), "policy": "retain repeated document-list observations; compare submitDateTime by normalized UTC instant; if any observation is preregistered docType 120/130, require one identical doc_type_code across all observations; selector performs deterministic doc_id dedup after sorting"},
...
def main() -> None:
    ...
    manifest["normalized_csv"] = {"path": str(metadata_path), "sha256": _sha256(metadata_path)}
