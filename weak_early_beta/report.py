...
<section class="panel" id="annual-pl"><h2>年別100株損益</h2><p class="note">モード別積み上げの集計です。同じ日・同じ銘柄が複数モードに該当した場合は、モードごとに100株の別取引として数えます。5営業日後の終値が確定した取引を検出年ごとに合計し、日次レポート更新時に数値も更新します。</p>{_annual_pl_chart(ledger, metrics, generated)}</section>
