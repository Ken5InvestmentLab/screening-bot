#!/usr/bin/env python3
"""V15 pre-2025 stable weak-day Tail rank (TEST ONLY).

Rule selection used 2023-2024 only.
...
2025 and 2026 are reporting only. No production writes.
"""
...
def select(tail: pd.DataFrame, trading_dates: pd.Index) -> pd.DataFrame:
    ...
    for c in RANK_FEATURES:
        ...
        rank_cols.append(rc)
    z["stable_weak_rank"] = z[rank_cols].mean(axis=1)
    z = z.sort_values(
        ["date", "stable_weak_rank", "tail_cdf", "tail_p"],
...
def main():
    ...
    report = {
        ...
        "component": "V15 pre-2025 stable weak-day V7 Tail rank",
        "feature_and_gate_selection_source": "2023-2024 only",
        "market_gates": [
            "med_ret5 <= 0",
            "breadth_ret1_pos <= 0.50",
        ],
        ...
        "pre2025_reference": {
            "2023H1_mean": 0.04616271127816208,
            "2023H2_mean": 0.020002690478155113,
            "2024H1_mean": 0.06309586706923259,
            "2024H2_mean": 0.0012749580036704865,
            "2023_2024_pooled_mean": 0.028788006667656258,
        },
        ...
        "warning": "2025/2026 are reporting only; do not tune V15 from these results.",
    }

    OUT.mkdir(parents=True, exist_ok=True)
    picks_2025.to_csv(OUT / "v15_weakday_2025_picks.csv", index=False)
    picks_2026.to_csv(OUT / "v15_weakday_2026_picks.csv", index=False)
    (OUT / "v15_weakday_report.json").write_text(
