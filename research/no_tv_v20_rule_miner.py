...
EXCLUDE_PREFIX=("teacher_","perf_","future_","published_","mode_","score_")
EXCLUDE_NAMES={"date","symbol","code","name","alert_id","received_at","session","stable_score","_published_stable_score","_snapshot_final"}


def as_bool(s):
    if s.dtype==bool:return s.fillna(False)
    z=s.astype(str).str.strip().str.lower()
    return z.isin(["1","true","yes","y"])


def detect_bool_columns(df):
    cols=[]
    for c in df.columns:
        if c in EXCLUDE_NAMES or c.startswith(EXCLUDE_PREFIX) or c.startswith("_"):continue
        s=df[c].dropna()
        if s.empty:continue
        u=set(s.astype(str).str.strip().str.lower().unique()[:20])
        if u and u.issubset({"0","1","true","false","yes","no","y","n"}):cols.append(c)
...
def objective(tr,va):
    ...
    core=min(tr["robust_avg"],va["robust_avg"])
    tail=.08*min(0.,va["p10"])+.02*min(0.,va["min"])
...
def mine(train,valid,all_cols,pool_n=16):
    ranked=single_rank(train,all_cols)
    pool=[c for _,c,_ in ranked[:pool_n]]
    for c in CURRENT:
    ...
    for k in (3,4,5,6):
        for combo in itertools.combinations(pool,k):
            checked+=1
            tr=stats(train[mask_for(train,combo)]);va=stats(valid[mask_for(valid,combo)]);o=objective(tr,va)
...
def main():
    ...
    df["perf_5bd"]=pd.to_numeric(df.perf_5bd,errors="coerce")
    ...
    for f in FOLDS:
        train=df[df.date_norm.between(f["train_start"],f["train_end"])&df.perf_5bd.notna()].copy()
        valid=df[df.date_norm.between(f["valid_start"],f["valid_end"])&df.perf_5bd.notna()].copy()
        test=df[df.date_norm.between(f["test_start"],f["test_end"])&df.perf_5bd.notna()].copy()
        best,meta=mine(train,valid,cols)
        tm=mask_for(test,best["conditions"]);cm=mask_for(test,CURRENT)
        tst=stats(test[tm]);cur=stats(test[cm])
        selected_parts.append(test[tm].assign(fold=f["id"]));current_parts.append(test[cm].assign(fold=f["id"]))
        folds.append({"fold":f["id"],"sizes":{"train":len(train),"valid":len(valid),"test":len(test)},"selected":best,"test":tst,"current_stable6_test":cur,"search":meta})
        print(f"{f['id']} best={best['conditions']} test={tst}",flush=True)
    sel=pd.concat(selected_parts,ignore_index=True) if selected_parts else df.iloc[0:0]
    cur=pd.concat(current_parts,ignore_index=True) if current_parts else df.iloc[0:0]
    result={"scope":"Actual Tenchi BOTTOM scoring-only rule mining. Boolean features are signal-time production snapshots; each fold mines train+prior validation only.","teacher_rows":len(df),"boolean_columns":cols,"folds":folds,"combined_test":{"candidate":stats(sel),"current_stable6_same_window":stats(cur)},"current_conditions":CURRENT}
