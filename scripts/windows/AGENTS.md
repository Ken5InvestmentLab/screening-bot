...
- Deploy to the local runtime through `install-premium-task.ps1`. Require a successful Session 0 Probe before staging Scheduled mode (disabled), and pause both desktop Premium automations before enabling production. Restore those automations if cutover fails.
