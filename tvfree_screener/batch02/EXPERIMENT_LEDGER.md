...
- Status: decisions frozen, reproduced, and recorded before outcome access in local commit `926f983`; candidate/rank/selection artifacts plus receipt are local `.cache` audit evidence and excluded from Git, while their hashes/counts are recorded here. Discovery evaluation is the next gated step. Weekly usage last observed 40% used / 60% remaining; pause latch remains false.
