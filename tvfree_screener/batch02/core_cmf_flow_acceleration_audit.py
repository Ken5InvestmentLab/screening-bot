"""Frozen purge-safe evaluation of the CMF flow-acceleration hypothesis."""
...
from tvfree_screener.batch02 import core_cmf_flow_acceleration as strategy
...
EXPERIMENT = "CORE-CMF-FLOW-ACCELERATION-20260913-01"
...
COST = 0.005
SPEC = REPORTS / "core_cmf_flow_acceleration_spec.json"
SPEC_SHA = REPORTS / "core_cmf_flow_acceleration_spec.sha256"
POOL = CACHE / "core_cmf_flow_acceleration_pool.parquet"
RANKED = CACHE / "core_cmf_flow_acceleration_ranked.parquet"
SELECTED = CACHE / "core_cmf_flow_acceleration_selected.parquet"
PREPARE = REPORTS / "core_cmf_flow_acceleration_pool_receipt.json"
REPRODUCE = REPORTS / "core_cmf_flow_acceleration_pool_reproduction.json"
REPORT = REPORTS / "core_cmf_flow_acceleration_discovery.json"
REPORT_MD = REPORTS / "core_cmf_flow_acceleration_discovery.md"
...
def implementation_hashes() -> dict[str, str]:
    files = {
        "candidate_selector": BATCH / "core_cmf_flow_acceleration.py",
...
def register() -> dict[str, object]:
    outputs = (SPEC, SPEC_SHA, POOL, RANKED, SELECTED, PREPARE, REPRODUCE, REPORT, REPORT_MD)
    if any(path.exists() for path in outputs):
        raise FileExistsError("CMF flow-acceleration experiment artifacts already exist")
    spec: dict[str, object] = {
        ...
        "historical_exposure": "2022H2 and 2023 outcomes were previously viewed for other experiments; this new family is retrospective discovery, not untouched OOS.",
        "hypothesis": "Positive and improving volume-weighted close-location pressure over five sessions relative to twenty sessions indicates sustained demand and may predict short-horizon continuation without requiring an opening gap or a one-day price breakout.",
        "candidate_pool": {
            "universe": "Valid daily OHLCV rows in the immutable feature-only XTKS panel. Historical membership is limited to symbols in the preserved Yahoo source; point-in-time delisted-symbol completeness is not guaranteed.",
            "bar_quality": "Finite positive OHLC prices and volume; high >= max(open, close), low <= min(open, close), and high > low.",
            "flow_windows": "Require complete, valid data on 20 consecutive official XTKS sessions ending at signal close. Any missing official session or invalid bar breaks the window; do not bridge gaps.",
            "money_flow_multiplier": "((close-low)-(high-close))/(high-low), equivalent to (2*close-high-low)/(high-low). Money-flow volume is multiplier times volume.",
            "cmf5_and_cmf20": "For each window, sum money-flow volume divided by sum volume, including the signal-day bar in both windows.",
            "event": "CMF5 > 0 and CMF5 > CMF20; no price-return, gap, market-regime, fundamental, or future-outcome filter.",
            "missingness": "Incomplete/nonfinite rolling windows are excluded by signal-time facts and counted; no session interpolation or outcome-dependent replacement.",
        },
        "score_and_selection": {
            "score": "CMF5 - CMF20, descending; no fitted parameters or threshold search.",
            "tie_breaks": ["CMF5 descending", "CMF20 descending", "symbol ascending"],
        ...
        "target": "Enter next official XTKS session open; exit close of fifth official XTKS session including entry, using pinned canonical labels and actionability statuses.",
        "periods": {"2022H2_last_signal": H2_LAST.date().isoformat(), "2023_last_signal": Y23_LAST.date().isoformat(), "last_discovery_exit": Y23_EXIT.date().isoformat(), "2024": "closed unless all discovery gates pass; retrospective confirmation only", "2025": "closed", "2026": "report-only and closed to selection"},
        "discovery_gates_each_period": {
            ...
            "tail_risk": "-10% and -20% rates no higher than the full event pool on the same selected dates",
...
def verify() -> tuple[dict[str, object], SessionCalendar]:
    if hashlib.sha256(SPEC.read_bytes()).hexdigest() != SPEC_SHA.read_text(encoding="utf-8").split()[0]:
        raise ValueError("CMF flow-acceleration spec hash mismatch")
    spec = json.loads(SPEC.read_text(encoding="utf-8"))
    if spec["input_sha256"] != input_hashes() or spec["implementation_sha256"] != implementation_hashes():
        raise ValueError("CMF code or data changed after preregistration")
    cm = json.loads(CALENDAR_MANIFEST.read_text(encoding="utf-8"))
...
def prepare() -> dict[str, object]:
    ...
    if any(path.exists() for path in (POOL, RANKED, SELECTED, PREPARE, REPRODUCE, REPORT, REPORT_MD)):
        raise FileExistsError("CMF flow-acceleration output artifacts already exist")
...
def reproduce() -> dict[str, object]:
    ...
    if receipt["artifact_sha256"] != {"pool": sha(POOL), "ranked": sha(RANKED), "selected": sha(SELECTED)}:
        raise ValueError("CMF flow-acceleration Parquet artifact hash mismatch")
    ...
    if hashes != receipt["decision_hashes"]:
        raise ValueError("CMF candidates/ranks/selections failed exact reproduction")
    result = {
        "experiment_id": EXPERIMENT,
        "spec_sha256": sha(SPEC),
...
def evaluate() -> dict[str, object]:
    ...
    if not reproduction.get("reproduced_exactly") or reproduction["decision_hashes"] != receipt["decision_hashes"]:
        raise ValueError("CMF candidate selection is not frozen and reproduced")
    ...
    if labels["date"].max() > Y23_LAST or labels["exit_date"].dropna().max() > Y23_EXIT:
        raise ValueError("CMF discovery labels escaped the frozen purge boundary")
    ...
    for name, begin, last in (("2022H2", START, H2_LAST), ("2023", pd.Timestamp("2023-01-01"), Y23_LAST)):
        ...
        active_pool_joined, active_pool_stats, active_pool_daily, active_pool_cohorts, _ = summarize(active_pool, labels, calendar.sessions, begin, last)
    ...
    decision = "KEEP_REQUIRES_SEPARATE_FROZEN_CONFIRMATION" if all(all(row.values()) for row in gates.values()) else "REJECT_CMF_FLOW_ACCELERATION"
    result = {
        "experiment_id": EXPERIMENT,
        "decision": decision,
        "evidence_level": spec["evidence_level"],
        "spec_sha256": sha(SPEC),
        "pool_receipt_sha256": sha(PREPARE),
        "pool_reproduction_sha256": sha(REPRODUCE),
        "decision_hashes": receipt["decision_hashes"],
        "canonical_label_digest": label_hash,
