...
.insight-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px;margin-bottom:22px}.insight-grid .panel{margin-bottom:0;min-width:0}.insight-grid .annual-pl-row{grid-template-columns:minmax(130px,190px) minmax(0,1fr) minmax(100px,125px);gap:8px}.payoff-heading{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}.payoff-ratio{background:color-mix(in srgb,var(--warn) 12%,var(--panel-solid));color:var(--warn);border-radius:999px;padding:5px 12px;font-weight:800;white-space:nowrap}.payoff-kpis{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin:20px 0}.payoff-kpi strong{display:block;font-size:clamp(22px,2.5vw,32px);line-height:1.25;font-variant-numeric:tabular-nums}.payoff-kpi.win strong{color:var(--good)}.payoff-kpi.loss strong{color:#c64c55}.payoff-track{position:relative;height:30px;border-radius:999px;background:color-mix(in srgb,var(--blue) 7%,var(--bg));overflow:hidden;margin-top:26px}.payoff-zero{position:absolute;left:50%;top:-8px;bottom:-8px;width:2px;background:var(--line)}.payoff-bar{position:absolute;top:0;bottom:0}.payoff-bar.win{left:50%;background:#14956d;border-radius:0 999px 999px 0}.payoff-bar.loss{right:50%;background:#df555a;border-radius:999px 0 0 999px}.payoff-axis{display:flex;justify-content:space-between;gap:12px;margin-top:7px;font-size:12px;font-weight:700}.payoff-axis .loss{color:#c64c55}.payoff-axis .win{color:var(--good)}
...
def _annual_pl_chart(ledger: pd.DataFrame, metrics: pd.DataFrame, generated: pd.Timestamp) -> str:
    ...
    for row in yearly.itertuples():
        ...
        rows.append(
            ...
            f'<div class="annual-pl-year">{label}<small>確定取引数 {int(row.n):,}件</small></div>'
            f'<div class="annual-pl-track" role="img" aria-label="{label}の100株損益 {_yen(value)}">'
...
def _payoff_structure(ledger: pd.DataFrame) -> str:
    """Summarize the completed stacked-trade win/loss distribution."""
    returns = pd.to_numeric(ledger["gross_return"], errors="coerce").dropna()
    if returns.empty:
        return '<p class="note">確定取引がありません。</p>'
    ...
    return (
        ...
        f'<div class="payoff-kpi win"><strong>{win_label}</strong><small>平均勝ち</small></div>'
        f'<div class="payoff-kpi loss"><strong>{loss_label}</strong><small>平均負け</small></div></div>'
