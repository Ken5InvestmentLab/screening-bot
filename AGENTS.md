...
`Deploy Approved Logic` 承認後のレポート再生成ではこのDiscord完了通知を送らない。`deploy.yml` から `mega-validation-report.yml` を呼ぶ場合は `notify_discord: false` を明示し、承認適用したスコアロジックの内容通知は `optimize_screener.py --apply-pending` 側で送る。
