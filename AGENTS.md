...
出力先はトップページ `reports/mega_validation_report_latest.html`、Markdown互換出力 `reports/mega_validation_report_latest.md`、モード別ページ `reports/mega_validation_report_<mode>.html`。ユーザー向けレポートとして、Stable ★6、Sniper 勝率重視、Mega5 短期リバウンド、Mega40 深押し反転、Mega40 下ヒゲ回復だけを対象にし、候補スコアカードや実装判断ゲートなどの内部向けセクションは出さない。トップページは全体成績とモード別サマリーに絞り、銘柄一覧はモードごとの別ページへ分け、全HTMLにハンバーガーメニューを置く。ハンバーガーメニューはトップと各モードページへの遷移だけにし、成績サマリー・確定済み全件・未確定ウォッチ・過去1年分などのページ内アンカーは入れない。通常表示の全体成績・モード別成績・銘柄一覧は `alerts_raw` のみで計算し、見出しには `alerts_rawのみ` を出さない。各モードページの任意表示セクションは「過去1年分の成績と銘柄を表示」とし、5営業日系は直近45営業日分、Mega40系は直近60営業日分を `alerts_raw + signals_archive` から別計算する。各モードでは確定済み成績と未確定ウォッチ全件を表示し、銘柄ごとにTradingViewチャートを出す。Discord投稿済みファンダ分析へのリンクは存在する銘柄だけに表示し、未連携ラベルや代替ファンダリンクは出さない。ボタン順はファンダ分析、チャートの順にする。Discord本文のHTML展開は `MEGA_REPORT_FETCH_DISCORD_MESSAGES=1` の明示時だけ実行し、通常生成ではDiscordメッセージリンクに留める。スマホでは銘柄テーブルを横スクロール前提にせず、カード状に読める表示を維持する。GitHub Actions の `Mega Validation Report` は optimizer 完了後と平日定期実行で同じレポートを再生成し、差分があれば `reports/` だけを自動コミットする。
