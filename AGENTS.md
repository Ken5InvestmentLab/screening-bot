...
公開するStable★数とSniper/Mega選定は `signal_feature_snapshots` の `alert_id` ごとの最初の `FINAL` を正として固定し、後続OHLCVや承認ロジックの変更で上書きしない。4時間足履歴の範囲内で、シグナル日より前の完了日に09:00/13:00足の片方または両方が欠ける場合だけ、その日全体の集約値をYahoo日足OHLCVへ置き換えて点灯時特徴量を確定する。部分4時間足と日足を混在させず、シグナル日には日足を使わず、補完値を `ohlcv_4h` へ書き戻さない。現在株価・将来騰落率だけは確定後も最新OHLCVから更新する。
Stable/Sniper/Megaの更新候補探索・採用判断・Discord比較は、HTMLと同じJST基準の直近365暦日だけを使う。現行集合は最初の `FINAL` に保存されたStable★数/モード所属を正とし、候補だけを同じ点灯時特徴量へ再適用する。勝率は0%の引き分けを分母から除外し、全履歴・後続OHLCV・旧評価ポリシーのpending/rescue stateを比較や承認に使わない。
...
サイトタイトルとトップH1は `天底極致 スコアリングレポート` にする。
...
テクニカル指標のユーザー向け説明は `CONDITION_LABELS` を共通の正とし、`close`、`Stoch`、`MACD hist`、`BB` などの内部・英語略記をそのまま出さず、「終値」「ストキャス」「MACDヒストグラム」「ボリンジャーバンド」など日本の投資家に馴染みやすい表記にする。
...
- **Stable最終置換ゲート**: rescue modeは候補探索だけを広げ、pending作成条件は通常時と同じく緩和しない。公開365日で勝率が現行比+8pt以上、平均が現行以上、件数が現行の70%以上かつモード別絶対下限以上を必須にする。検証は現行件数の80%以上、Lockboxは8件以上かつ現行件数の70%以上を維持し、どちらも勝率・平均を悪化させない。さらにBootstrap CI下限が現行勝率超、K-Foldが2/3以上、Permutation Testが`p < 0.05`でなければ、rescue中でもpending・承認通知・デプロイを行わない。
...
- **差分品質ゲート**: スコアロジック更新候補をpending保存・Discord通知する直前に、現行/候補の抽出銘柄差分を確認する。候補件数が現行比で大きく減り、候補のみ追加の件数や成績が弱く、現行のみ除外側に目標Hitや大勝ち銘柄がある場合は、見かけの成績改善として自動却下する。検証/Lockbox、全体成績、未確定ウォッチリストの改善が十分に強い場合だけ通し、rescue modeでもこの差分品質ゲートを省略しない。
- **採用強制ゲート**: Stable/Sniper/Mega候補は、pending保存前に未確定平均、-10%以下件数、中央値、直近Lockbox（Megaは直近検証期間）を現行と比較し、悪化する候補を自動却下する。Mega候補は勝率悪化を平均騰落率の改善だけで補わず、平均改善の大半が最大1銘柄に依存し、その銘柄を除いた平均・中央値・目標Hit率に十分な改善がなければ、pending保存とDiscord承認申請の前に自動却下する。`body_pullback10`（実体10%以上）と `body_overheat15`（実体15%以上）は過熱系の探索候補であり、採用には同じ強制ゲート通過が必要。
...
- **全モード公開値ゲート**: Stable/Sniper/Megaの候補は条件を全履歴へ再適用した参考試算だけで採用せず、first-FINAL所属・Stable★数と未確定行の条件フォールバックを使う公開HTMLと同じ集合で勝率が現行より悪化しないことをpending保存・承認通知の必須条件にする。承認通知と完了通知の主成績は公開HTML集計を使い、全履歴再適用値を載せる場合は参考試算と明記する。
...
- **Daily Screener Optimizationのモード分離**: 日次実行ではMega3種を `--propose-mega-report-logic-only`、Sniperを `--propose-sniper-only` でStable探索から独立して必ず提案探索する。Stableの `--win-threshold-sweep` と最終 `--propose` 子プロセスはSniper重複通知を避けるため `--skip-sniper` のままにする。提案実行では承認済みの `current_logic*.json` を更新せず、承認待ちの `pending_logic*.json` と必要な `rescue_state.json` だけを変更対象にする。
- **承認通知の順序**: Stable/Sniper/MegaのDiscord承認リクエストは各探索直後に送らず、全探索完了後に `pending_logic*.json` をcommit・pushしてからまとめて送る。承認適用時は提案時と同じ評価で通過済みの候補を古い `current_logic*.json` 保存成績で再判定せず、提案時の現行条件が後から変わっていないことだけを確認する。
