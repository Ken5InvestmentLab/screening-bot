#!/usr/bin/env python3
"""V16 simplest pre-2025 stable Tail rank (TEST ONLY).

Rule discovery used 2023-2024 only.

Frozen rule:
- V7 extreme Tail candidates (cdf >= 0.999)
- market gate: med_ret5 <= 0
- among same-day Tail candidates prefer lower volr20
- tie-break by higher Tail CDF / Tail probability
- one-business-day same-symbol cooldown

volr20-low was the strongest single feature that stayed positive in all four
pre-2025 half-years. 2025/2026 are reporting only.
"""
...
def main():
    ...
    report={
        ...
        "component":"V16 pre-2025 stable single-feature V7 Tail rank",
        "selection_source":"2023-2024 only",
        "market_gate":"med_ret5 <= 0",
        "rank_feature":"volr20 low",
        "pre2025_reference":{
            "2023H1_mean":0.0028015774414176836,
            "2023H2_mean":0.0010600270022228186,
            "2024H1_mean":0.037337769872055504,
            "2024H2_mean":0.02330981684096117,
            "2023_2024_pooled_mean":0.018065416868875226,
        },
        "2025":pstats(p25,{
        ...
        "warning":"2025/2026 are reporting only; do not tune V16 from these results.",
    }
    OUT.mkdir(parents=True,exist_ok=True)
    p25.to_csv(OUT/"v16_volr20_2025_picks.csv",index=False)
    p26.to_csv(OUT/"v16_volr20_2026_picks.csv",index=False)
    (OUT/"v16_volr20_report.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
