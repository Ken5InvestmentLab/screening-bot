...
| EDINET same-ZIP cross-check | 🟢 **再実行中 / null tombstone対応済み** | **45%** | 2023/2024/2025 metadata取得自体は全成功。初回freezeはEDINET公式仕様にある`docTypeCode=null / submitDateTime=null`の取下げ・無効化placeholderで停止。厳密なtombstone形だけ除外し、それ以外はfail-closedのまま修正。OSS回帰testとfull freeze retry `34922234308` 実行中 |
