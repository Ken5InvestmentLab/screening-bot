...
Mega5/Mega40 の条件は `current_logic_mega.json` を正とし、`Daily Screener Optimization` 内の `optimize_screener.py --update-mega-report-logic-only` が更新する。これはHTML/Markdownレポート専用で、Bot本体の `screener.js` / `index.js` や `/scan` コマンドにMegaモードを追加しない。
